...
- [ ] T019 Run quickstart.md verification: execute scenarios 1–14 in order and record PASS/FAIL for each. **M2 fix — timing**: for scenarios 5 (Linux base build) and 8 (Pi base build), record wall-clock time and confirm they fall within SC-001 (<60 min) and SC-002 (<90 min) budgets. For scenarios 6 and 9, record the artifact file size and confirm >50 MB. Update `specs/013-docker-platform-builds/checklists/build-pipeline.md` to mark items that were validated [X].
  > **Status 2026-07-09**: deferred (requires Docker Desktop) and superseded in practice — the phase goal was achieved via GitHub Actions instead: release v1.0.0 shipped all 5 platform artifacts from CI (win64-setup.exe, linux AppImage, 2× macOS dmg, Pi arm64.deb on a native ARM64 runner). The local Docker path remains available but unverified; note that PyInstaller torch collection segfaults under QEMU, so the Pi Docker path likely needs the same native-ARM approach if ever exercised.
