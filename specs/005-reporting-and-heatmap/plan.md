...
**Performance Goals**: Full report generation in ~10s (best-effort, measured 9-12s on the reference dev machine) for a job with 50 included events or fewer (SC-P5-001; not guaranteed beyond that count, but expected to scale proportionally rather than fail or hang); heatmap accumulation adds negligible per-frame overhead next to MOG2/YOLO inference itself
