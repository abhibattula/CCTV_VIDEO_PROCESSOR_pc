...
- **FR-011**: The intelligence report MUST include a static SVG activity timeline showing when events occurred across the video duration — it renders identically in both HTML and PDF with no interactive behaviour
...
- **FR-013**: When an LLM API key is available, the executive summary MUST be generated by the language model using the full event list as context; when unavailable or when the API call fails/times out, rule-based synthesis MUST be used and the report MUST display a one-line notice (e.g., "Executive summary: rule-based synthesis — LLM API unavailable") so the user knows which path ran
...
- Q: When LLM API falls back to rule-based synthesis, should the user be notified in the report? → A: Yes — show a one-line notice in the report header so the user knows which path ran (FR-013 updated)
- Q: Should the Activity Timeline SVG in the HTML report be interactive (hover tooltips) or static? → A: Static SVG only — renders identically in HTML and PDF, no JS required (FR-011 updated)
